      ...
      .then((data) => {
        ...
        setStats([
          ...
          },
          {
            ...
            value: `$${revenueSummary.avgRevenuePerTrip.toFixed(2)}`,
            icon: <FaChartBar />,
            color: "blue",
          },
          {
            label: "Completed Trips",
            value: revenueSummary.totalTrips.toLocaleString(),
  ...
  useEffect(() => {
    const fetchRevenueAnalytics = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:3000/api/analytics/revenue?${params.toString()}`);
        ...
        if (isSingleDay && analytics.length === 1) {
          ...
          // quick date format check YYYY-MM-DD
          if (/^\d{4}-\d{2}-\d{2}$/.test(singleLabel)) {
            const hourly = Array.from({ length: 24 }, (_, i) => ({ label: `${i}:00`, revenue: 0, trips: 0 }));
            const total = analytics[0].revenue || 0;
            // place total at 12:00 for visibility (non-destructive fallback)
            hourly[12].revenue = total;
            hourly[12].trips = analytics[0].trips || 0;
            analytics = hourly;
          }
        }

        setRevenueAnalytics(analytics);
        setRevenueSummary(data.summary || { totalRevenue: 0, totalTrips: 0, avgRevenuePerTrip: 0 });
      } catch (err) {
